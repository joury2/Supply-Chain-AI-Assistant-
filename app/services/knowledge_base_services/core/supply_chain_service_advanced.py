# app/services/knowledge_base_services/core/supply_chain_service_advanced.py
...
class SupplyChainForecastingService:
    ...
    # from def __int__ argument i removed config: PipelineConfig = None
    ...
    def _generate_cache_key(self, data: Dict[str, Any]) -> str:
        ...
        for key, value in data.items():
            # Skip DataFrames, functions, and other non-serializable types
            if key in ['data', 'df', 'dataframe']:
                continue
            if isinstance(value, (str, int, float, bool, type(None))):
                clean_data[key] = value
            elif isinstance(value, (list, tuple)):
                # Only include if items are serializable
                try:
    ...
    def analyze_dataset_with_knowledge_base(self, dataset_info: Dict[str, Any]) -> Dict[str, Any]:
        ...
        FIXED: Handles DataFrame serialization correctly
        """
        ...
        if self.config.get('enable_caching', True):
            cache_key = self._generate_cache_key(dataset_info)  # ✅ Now safe to hash
            if cache_key in self._analysis_cache:
                ...
                if time.time() - cache_entry['timestamp'] < cache_ttl:
                    ...
                    # ✅ Restore DataFrame to result (optional, usually not needed in cache)
        ...
        if self.config.get('enable_caching', True):
            # ✅ Remove DataFrame before caching
