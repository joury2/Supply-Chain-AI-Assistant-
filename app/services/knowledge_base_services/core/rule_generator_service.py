...
class RuleTemplates:
    ...
    TEMPLATES = {
        'feature_based': {
            ...
            'condition': " and ".join(["'{{feature}}' in dataset.columns" for feature in "{features}"]),
            'action': "SELECT model_id FROM ML_Models WHERE model_name = '{model_name}'",
            'priority': 9,
        ...
        'frequency_based': {
            ...
            'condition': "dataset.frequency == '{frequency}'",
            'action': "SELECT model_id FROM ML_Models WHERE model_name = '{model_name}'", 
            'priority': 8,
        ...
        'data_quality_based': {
            ...
            'condition': "dataset.row_count >= {min_rows} and dataset.missing_percentage <= {max_missing}",
            'action': "SELECT model_id FROM ML_Models WHERE model_name = '{model_name}'",
            'priority': 7,
        ...
        'target_based': {
            ...
            'condition': "'{target_variable}' in dataset.columns and dataset.row_count >= {min_rows}",
            'action': "SELECT model_id FROM ML_Models WHERE model_name = '{model_name}'",
            'priority': 8,
        ...
        for key, value in rule.items():
            if isinstance(value, str):
                ...
                if key == 'condition' and '{features}' in value:
                    ...
                    feature_conditions = " and ".join([f"'{feature}' in dataset.columns" for feature in features])
                    rule[key] = value.replace(" and ".join(["'{feature}' in dataset.columns" for feature in "{features}"]), feature_conditions)
                else:
...
class RuleGeneratorService:
    ...
    def _test_action_validity(self, action: str) -> bool:
        """Test if action syntax is valid"""
        # Basic SQL syntax check
        if not action.strip().upper().startswith('SELECT'):
            raise ValueError("Action must be a SELECT SQL statement")
        
        # Check for required model_name reference in WHERE clause
        if 'model_name' not in action and 'model_id' not in action:
            raise ValueError("Action must reference model_name or model_id in WHERE clause")
    ...
    def _check_rule_warnings(self, rule: Dict[str, Any]) -> List[str]:
        ...
        # Check for missing error handling in action
        if 'LIMIT 1' not in rule['action'] and 'COUNT' not in rule['action']:
            warnings.append("Action should include LIMIT 1 to ensure single result")
    ...
    def _generate_lightgbm_rules(self, model_name: str, required_features: List[str]) -> List[Dict[str, Any]]:
        ...
        if has_retail_features:
            ...
            rules.append({
                ...
                'action': f"SELECT model_id FROM ML_Models WHERE model_name = '{model_name}'",
                'priority': 10,
        ...
        return [
            {
                ...
                'condition': "dataset.frequency in ['daily', 'weekly', 'monthly'] and dataset.row_count >= 100",
                'action': f"SELECT model_id FROM ML_Models WHERE model_name = '{model_name}'",
                'priority': 9,
    ...
    def _generate_timeseries_rules(self, model_name: str) -> List[Dict[str, Any]]:
        ...
        return [{
            ...
            'condition': "dataset.frequency != 'none' and 'date' in dataset.columns",
            'action': f"SELECT model_id FROM ML_Models WHERE model_name = '{model_name}'",
            'priority': 8,
    ...
    def _generate_regression_rules(self, model_name: str, required_features: List[str]) -> List[Dict[str, Any]]:
        ...
        if len(required_features) >= 5:
            rules.append({
                ...
                'condition': f"len(dataset.columns) >= 8 and dataset.row_count >= 500",
                'action': f"SELECT model_id FROM ML_Models WHERE model_name = '{model_name}'",
                'priority': 8,
...
def demo_enhanced_rule_generator():
    ...
    print("🚀 ENHANCED RULE GENERATOR WITH VALIDATION & OPTIMIZATION")
    print("=" * 60)
